...
- Layer caps default to a soft budget of 1 000 shapes/connectors. Override via diagram metadata (`layout.layers.maxShapes`, `layout.layers.maxConnectors`) or the CLI switches (`--layer-max-shapes`, `--layer-max-connectors`) only when you need tighter layer splits. In view mode, the connector cap is effectively unlimited so the planner doesn’t fragment layers unless you explicitly lower the limit. Soft breaches emit warnings; hard breaches log `LayerOverflow` issues.
