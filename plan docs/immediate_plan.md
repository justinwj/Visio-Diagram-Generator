...
4. **CLI render loop** – Drive Visio with the new payload until the subroutine layout is visually correct (connectors land in the right slots, no skipped edges). Capture diagnostics + screenshots.
5. **Regression hooks** – Add/adjust unit tests around the planner output for this subroutine and wire the fixture into the existing render scripts so we can grow from this baseline.
